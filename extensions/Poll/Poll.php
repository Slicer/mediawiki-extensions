<?php
# Version 1.4 - Tomas Jensen / 2008
# Version 1.3 - Sorin Sbarnea / 2008
# Version 1.2 - Eric David / 2006
...
function renderPoll( $input, $argv=array() ) {
  ...
  // Only logged in users can submit votes
  ...
  if (!empty($_POST['poll_ID']))  // POST Variables treatments
    {
      ...
      $_POST['comment'] = ereg_replace("''","'",$_POST['comment']); // sinon effet cumul+¬
      }
    }
...
//             "memory_limit : ".ini_get('memory_limit')."</font><BR>";
    ...
    if ($lines[1] == "STATSFR") // special param : stats
    {
      ...
      $tab = mysql_fetch_array($result);
      ini_set( 'memory_limit',"20M");
      return "Il y a actuellement $tab[1] sondages et $tab[0] opinions Tmises par $tab[2] internautes.".
      "<br>La derniFre opinion a TtT Tmise a la date <i>$tab[3]</i>.<BR>";
//      ."<font size=1>( memory_get_usage() : ".$memory."    ".
//             "memory_limit : ".ini_get('memory_limit').") </font><BR>";
    }
    // Getting the votes
    ...
    for ($i=2;$i<$nbansw;$i++)
      ...
      '<input '.$disabled.' type="submit" value="+"> '.$lines[$i].'  </form></td>'."\n";
      if ($total>0)
        {
        ...
        $a = $deja_vote==1?"a":"b";
        if ($res>0)
        $str .= '<img src="'.$imagePath.'left'.$a.'.gif">'.
        '<img src="'.$imagePath.'middle'.$a.'.gif" alt="'.$res.' votes" height=10 width='.round($res*100/$total).'>'.
        '<img src="'.$imagePath.'right'.$a.'.gif">';
